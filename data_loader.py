...
def load_data(args):
    n_user, n_item,  list_users, list_items, train_data, eval_data, test_data = load_rating(args)
    n_entity, n_relation, adj_entity, adj_relation = load_kg(args)
    return n_user, n_item, list_users, list_items, n_entity, n_relation, train_data, eval_data, test_data, adj_entity, adj_relation
...
def load_rating(args):
    ...
    list_users = np.unique(rating_np[:, 0])
    list_items = np.unique(rating_np[:, 1])
    train_data, eval_data, test_data = dataset_split(rating_np, args)
    return n_user, n_item, list_users, list_items, train_data, eval_data, test_data
