...
class TransE(nn.Module):
    ...
    def _embedding(self, triplets):
        assert triplets.size()[1] == 3
        heads = triplets[:, 0]
        relations = triplets[:, 1]
        tails = triplets[:, 2]
        # print(relations)   # tensor([ 2,  2,  2,  ..., 20, 20, 20], device='cuda:0')
        # print(type(relations))  # <class 'torch.Tensor'>

        return self.entities_emb(heads), self.relations_emb(relations), self.entities_emb(tails)
    
    
    def embdding_entity(self, entitys):
        return self.entities_emb(entitys)
    
    # def embedding_relaton(self, relatons):
    def embdding_relation(self, relatons):
        return self.relations_emb(relatons)
